...
const CollectionsImage: React.FC<CollectionsImageProps> = ({ desiredBlocks = 50, alt = "Pixelated Image" }) => {
  const { attempts, isSuccessful, currentImageData } = useCollectionsContext();
  ...
  const src = currentImageData ? currentImageData.url : '';
      ...
      for (let y = 0; y < h; y += sampleSize) {
        for (let x = 0; x < w; x += sampleSize) {
          if (shouldPixelate(x, y, w, h, guessNumber)) {
            ...
            // Corrected alpha value to be between 0 and 1
